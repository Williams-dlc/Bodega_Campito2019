    ...
    public partial class ReportesEntradas : Form
    {
        ...
        public void Barras_producto()
        {
            ...
            using (OdbcConnection con = new OdbcConnection(ConnStr))
            {
                ...
                OdbcDataAdapter cmd = new OdbcDataAdapter("SELECT ANY_VALUE(d.fk_encEntrada) AS 'Codigo', p.name, SUM(Cantidad) AS 'Cantidad' from DetalleEntrada as d, Producto as p, EncabezadoEntrada as b where d.FK_encEntrada=b.idEntrada AND b.FK_Distribuidor='Jorge' AND b.Fecha BETWEEN '2020-01-1' AND '2020-01-30' and p.idProducto=d.Fk_Producto GROUP BY d.Fk_Producto", con);//llama a la tabla de inventario para ver stock
        ...
        public void pie_producto()
        {
            ...
            {
                con.Open();
                OdbcDataAdapter cmd = new OdbcDataAdapter("SELECT ANY_VALUE(d.fk_encEntrada), p.name, SUM(d.Cantidad) AS 'Cantidad' from Detalleentrada d, Producto p, EncabezadoEntrada b where d.FK_encEntrada=b.idEntrada AND b.FK_Distribuidor='" + cmb_propietario.Text.ToString() + "' AND b.Fecha BETWEEN '" + cmb_year.Text.ToString() + "-" + txt_numero.Text + "-1' AND '" + cmb_year.Text.ToString() + "-" + txt_numero.Text + "-30' and p.idProducto=d.Fk_Producto GROUP BY Fk_Producto", con);//llama a la tabla de inventario para ver stock
                                                                                                                                                                                     //OdbcDataReader queryResults = cmd.ExecuteReader();
        ...
        public void Barras_productoAnual()
        {
            ...
            {
                con.Open();
                OdbcDataAdapter cmd = new OdbcDataAdapter("SELECT ANY_VALUE(d.fk_encEntrada), p.name, SUM(d.Cantidad) AS 'Cantidad' from DetalleEntrada d, Producto p, EncabezadoEntrada b where d.FK_encEntrada=b.idEntrada AND b.FK_Distribuidor='" + cmb_propietario.Text.ToString() + "' AND b.Fecha BETWEEN '" + cmb_year.Text.ToString() + "-1-1' AND '" + cmb_year.Text.ToString() + "-12-31' and p.idProducto=d.Fk_Producto GROUP BY Fk_Producto", con);//llama a la tabla de inventario para ver stock
                                                                                                                                                                                     //OdbcDataReader queryResults = cmd.ExecuteReader();
                                                                                                                                                                                     //SELECT YEAR(Fecha), SUM(Cantidad) as total, '"+cmb_propietario.Text.ToString()+"' from encabezadoentrada a INNER JOIN detalleentrada b on a.idEntrada = b.FK_encEntrada
                                                                                                                                                                                     //"select * from detalleInventario where FK_Propietario =  '"+cmb_propietario.Text.ToString()+"' and Fecha=MONTH(text)"
                cmd.Fill(tabla);
        ...
        public void Barras_productoSalidas()
        {
            ...
            {
                con.Open();
                OdbcDataAdapter cmd = new OdbcDataAdapter("SELECT ANY_VALUE(d.FK_EncPedido) as 'Codigo', p.name as 'Producto', SUM(d.Cantidad) AS 'Cantidad' from DetallePedido d, Producto p, EncabezadoPedido b where d.FK_EncPedido=b.idPedido AND b.FK_Usuario='" + cmb_propietario.Text.ToString() + "' AND b.Fecha BETWEEN '" + cmb_year.Text.ToString() + "-" + txt_numero.Text + "-1' AND '" + cmb_year.Text.ToString() + "-" + txt_numero.Text + "-30' and p.idProducto=d.Fk_Producto GROUP BY Fk_Producto", con);//llama a la tabla de inventario para ver stock
                                                                                                                                                                                                                                                                                                                                                                                                     //OdbcDataReader queryResults = cmd.ExecuteReader();
                                                                                                                                                                                                                                                                                                                                                                                                     //SELECT YEAR(Fecha), SUM(Cantidad) as total, '"+cmb_propietario.Text.ToString()+"' from encabezadoentrada a INNER JOIN detalleentrada b on a.idEntrada = b.FK_encEntrada
                                                                                                                                                                                                                                                                                                                                                                                                     //"select * from detalleInventario where FK_Propietario =  '"+cmb_propietario.Text.ToString()+"' and Fecha=MONTH(text)"
                cmd.Fill(tabla);
