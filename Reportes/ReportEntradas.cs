    ...
    public partial class ReportEntradas : Form
    {
        ...
        public void productoMensual()
        {
            ...
            using (OdbcConnection con = new OdbcConnection(ConnStr))
            {
                ...
                OdbcDataAdapter cmd = new OdbcDataAdapter("SELECT d.fk_encEntrada as 'Codigo', p.name, SUM(Cantidad) AS 'Cantidad' from detalleentrada d, producto p, encabezadoentrada b where d.FK_encEntrada=b.idEntrada AND b.FK_Distribuidor='" + cmb_propietario2.Text.ToString() + "' AND b.Fecha BETWEEN '" + cmb_year.Text.ToString() + "-" + txt_numero.Text + "-1' AND '" + cmb_year.Text.ToString() + "-" + txt_numero.Text + "-30' and p.idProducto=d.Fk_Producto GROUP BY Fk_Producto", con);//llama a la tabla de inventario para ver stock
                cmd.Fill(tabla);
